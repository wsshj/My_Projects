...
def select_to_json(responses):
    ...
    for response in responses:
        response['staff_name'] = Staff.objects.filter(id=response['staff_id']).values().first()['name']
        ...
        response['task_name'] = Task.objects.filter(id=response['task_id']).values().first()['name']
...
def select_monthly(request):
    ...
    data = {}
    data['response'] = project_fun(list(dailies))
